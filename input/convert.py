...
binary = '/srv/bbb-convert/bbb-convert'
...
with open(file_path, 'r') as file:
    ...
    for line in file:
        ...
        #result = subprocess.run([binary, '-i', basedir + '/presentation/' + bbb_id, '-o', file_name.replace('.webm', '.mp4')], capture_output=True, text=True)
        result = subprocess.run([binary, '-i', basedir + '/presentation/' + bbb_id, '-t', '4', '-o', file_name], capture_output=True, text=True)
