...
func CombinePresentationWithWebcams(presentation Video, webcam Video, config config.Data) (Video, error) {
	videoPath := path.Join(config.WorkingDir, "out.mkv")
...
func copyWebcamsVideo(webcam Video, videoPath string, config config.Data) error {
	_, err := util.ExecuteCommand("ffmpeg", "-hide_banner", "-loglevel", "error", "-threads", config.ThreadCount, "-i", webcam.VideoPath, "-c:v", "copy", "-y", videoPath).Output()
	if err != nil {
		return err
	}
	return nil
...
func copyWebcamsAudioToPresentation(presentation Video, webcam Video, videoPath string, config config.Data) error {
	_, err := util.ExecuteCommand("ffmpeg", "-hide_banner", "-loglevel", "error", "-threads", config.ThreadCount, "-i", presentation.VideoPath, "-i", webcam.VideoPath, "-c:v", "copy", "-c:a", "flac", "-map", "0:0", "-map", "1:1", "-shortest", "-preset", "ultrafast", "-y", videoPath).Output()
	if err != nil {
		return err
	}
	return nil
